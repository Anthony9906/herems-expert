...
class TestAgentExecution:
    ...
    @pytest.mark.asyncio
    async def test_run_agent_triggers_auto_title_for_successful_api_server_session(self, adapter):
        mock_agent = MagicMock()
        mock_agent.run_conversation.return_value = {
            "final_response": "The dashboard title is generated by Hermes.",
            "messages": [
                {"role": "user", "content": "为什么 dashboard 没有标题？"},
                {"role": "assistant", "content": "The dashboard title is generated by Hermes."},
            ],
        }
        ...
        mock_agent.base_url = "https://example.invalid/v1"
        mock_agent.api_key = "test-key"
        mock_agent.api_mode = "chat_completions"
        mock_agent._emit_auxiliary_failure = MagicMock()
        session_db = MagicMock()
        adapter._session_db = session_db

        with (
            patch.object(adapter, "_create_agent", return_value=mock_agent),
            patch("agent.title_generator.maybe_auto_title") as mock_auto_title,
        ):
            result, usage = await adapter._run_agent(
                user_message="为什么 dashboard 没有标题？",
                conversation_history=[],
                session_id="session-123",
            )

        assert result["final_response"] == "The dashboard title is generated by Hermes."
        assert usage == {"input_tokens": 1, "output_tokens": 2, "total_tokens": 3}
        mock_auto_title.assert_called_once_with(
            session_db,
            "session-123",
            "为什么 dashboard 没有标题？",
            "The dashboard title is generated by Hermes.",
            mock_agent.run_conversation.return_value["messages"],
            failure_callback=mock_agent._emit_auxiliary_failure,
            main_runtime={
                "model": "test-model",
...
class TestChatCompletionsEndpoint:
    ...
    @pytest.mark.asyncio
    async def test_stream_includes_reasoning_content_delta(self, adapter):
        """reasoning_callback emits OpenAI-compatible delta.reasoning_content chunks."""
        import asyncio
        ...
        async with TestClient(TestServer(app)) as cli:
            async def _mock_run_agent(**kwargs):
                ...
                cb = kwargs.get("stream_delta_callback")
                if reasoning_cb:
                    reasoning_cb("I should inspect the working directory.")
                if cb:
                    await asyncio.sleep(0.05)
                    cb("The answer is ready.")
                return (
                    {"final_response": "The answer is ready.", "messages": [], "api_calls": 1},
                    {"input_tokens": 10, "output_tokens": 5, "total_tokens": 15},
                )
            ...
            with patch.object(adapter, "_run_agent", side_effect=_mock_run_agent):
                ...
                assert reasoning_chunks == ["I should inspect the working directory."]
                assert "The answer is ready." in content_chunks
